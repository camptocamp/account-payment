...
class AccountPaymentRegister(models.TransientModel):
    ...
    def prepare_group_data_values(self, line):
        """Return an update to apply to group data"""
        line.payment_difference = line.balance - line.amount
        partner_id = line.invoice_id.partner_id.id
        return {partner_id: self.prepare_partner_id_values(partner_id, line)}
    ...
    def make_payments(self):
        ...
        for invoice_payment_line in self.invoice_payments:
            if invoice_payment_line.amount > 0:
                self.prepare_group_data_values(invoice_payment_line)
        # update context
